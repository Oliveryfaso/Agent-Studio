...
if [[ ($# -ne 3 && $# -ne 5) || ("$1" != "codex" && "$1" != "claude-code") ]]; then
  echo "Usage: scripts/run-analyze.sh codex <authorized-workspace> <current-directory> [--codex-config <snapshot.toml>]" >&2
  echo "   or: scripts/run-analyze.sh claude-code <authorized-workspace> <current-directory> [--target-file <project-relative-file>]" >&2
  exit 2
fi
...
task_build_dir="$repo_root/build/core-analyze/classes"

case "$task_build_dir" in
  "$repo_root"/build/core-analyze/classes) ;;
...
source_list="$repo_root/build/core-analyze/sources.txt"
...
java -cp "$task_build_dir" dev.agentconfig.workbench.cli.Main analyze "$@"
