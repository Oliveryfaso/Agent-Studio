...
if [[ $# -ne 1 ]]; then
  echo "Usage: scripts/run-skill-blueprint-preview.sh <guided-request.intent>" >&2
  exit 2
fi
...
request_file=$1
task_build_dir="$repo_root/build/core-skill-blueprint-preview/classes"
...
case "$task_build_dir" in
  "$repo_root"/build/core-skill-blueprint-preview/classes) ;;
...
source_list="build/core-skill-blueprint-preview/sources.txt"
...
java -cp "$task_build_dir" dev.agentconfig.workbench.cli.Main \
  skill-blueprint-preview codex < "$request_file"
