...
if [[ $# -ne 1 ]]; then
  echo "Usage: scripts/run-local-api.sh <existing-trusted-state-root>" >&2
  exit 2
fi
...
task_build_dir="$repo_root/build/core-local-api/classes"

case "$task_build_dir" in
  "$repo_root"/build/core-local-api/classes) ;;
...
cd -- "$repo_root"
source_list="build/core-local-api/sources.txt"
