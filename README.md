...
- Vue 3 + TypeScript 单页已覆盖 workspace + Skill 中文表单、真实 Diff、明确批准、apply receipt 与 guarded rollback。表单会确定性生成既有 guided request 协议，提供完整可运行示例和即时完整性检查；高级配置仍可直接编辑原始 `key: value`。输入变化会立即废弃旧 preview/approval，`BLOCKED`、`NO_CHANGE`、`STALE`、`CURRENT_TARGET_CHANGED`、`RECOVERY_REQUIRED` 与网络错误分开呈现。启动 token 通过 URL fragment 交付，页面读取后立刻从地址栏删除并只保留在内存。
- Java 同源静态资源服务只允许构建产物中的 `index.html`、hash asset 与 favicon，带 CSP/no-store/nosniff 等响应头；仍只绑定 loopback，state root 仍由启动参数固定。
- Ubuntu、macOS、Windows 三平台 CI 基线已真实通过并持续复验；当前本地 271 项 Java 测试用例全部通过，其中版本化 conformance 为 27 项。每次变更仍以对应远端 CI run 为合并依据。
