...
- This project is a lab prototype whose active delivery path is Codex-first: `Inspect -> Draft -> Diff/Export -> Simple Apply/Rollback`. The read-only scanner, Codex/Claude effective-context compiler, Instruction IR analyzer, and narrow conversion preview remain the deterministic governance kernel. Codex `inspect`, S0 project Skill inventory, and the S1 deterministic persistence-triage / `SkillBlueprint v1` preview now exist. Candidate authoring, Diff/Export, target writes, rollback, Vue UI, and optional AI remain unimplemented; generic conversion, additional hosts, router, and history evolution are frozen backlog until the core loop is validated.
