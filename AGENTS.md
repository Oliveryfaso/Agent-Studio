...
- This project is a lab prototype whose active delivery path is Codex-first: `Inspect -> Draft -> Diff/Export -> Simple Apply/Rollback`. Codex `inspect` and Skill S0 inventory, S1 persistence triage / `SkillBlueprint v1`, and S2 deterministic in-memory `SKILL.md` candidate / static validation / stdout export now exist. S2 never receives or writes a target workspace; its Diff is synthetic new-file output. The next gate is single-file target review and Simple Apply/Rollback. Vue, optional AI, generic conversion, additional hosts, router, and history evolution remain unimplemented or frozen backlog.
