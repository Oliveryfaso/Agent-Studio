...
- This project is a lab prototype whose active delivery path is Codex-first: `Inspect -> Draft -> Diff/Export -> Simple Apply/Rollback`. The read-only scanner, Codex/Claude effective-context compiler, Instruction IR analyzer, and narrow conversion preview remain the deterministic governance kernel. A human-readable, zero-write Codex `inspect` entry now exists. Skill authoring, Diff/Export, target writes, rollback, Vue UI, and optional AI remain unimplemented; generic conversion, Git/GitHub, additional hosts, router, and history evolution are frozen backlog until the core loop is validated.
