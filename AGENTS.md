...
- This project is a lab prototype whose active delivery path is Codex-first: `Inspect -> Draft -> Diff/Export -> Simple Apply/Rollback`. The read-only scanner, Codex/Claude effective-context compiler, Instruction IR analyzer, and narrow conversion preview remain the deterministic governance kernel. A human-readable, zero-write Codex `inspect` entry and the first S0 project Skill inventory slice now exist. Persistence triage, SkillBlueprint, candidate authoring, Diff/Export, target writes, rollback, Vue UI, and optional AI remain unimplemented; generic conversion, additional hosts, router, and history evolution are frozen backlog until the core loop is validated.
...
- Run `scripts/run-skill-inventory.sh <fixture-root>` for Codex project Skill inventory schema v1. It may read only bounded `SKILL.md` frontmatter, must never read or execute supporting-file content, must reject symlinked Skill paths, and must keep `contentIncluded=false` and `writesPerformed=false`.
